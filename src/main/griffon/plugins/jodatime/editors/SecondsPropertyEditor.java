 ...
 */
public class SecondsPropertyEditor extends AbstractPropertyEditor {
    protected void setValueInternal(Object value) {
        if (null == value) {
            super.setValueInternal(null);
        } else if (value instanceof CharSequence) {
            handleAsString(String.valueOf(value));
        } else if (value instanceof Seconds) {
            super.setValueInternal(value);
        } else if (value instanceof Number) {
            super.setValueInternal(parse((Number) value));
        } else {
            throw illegalValue(value, Seconds.class);
        }
    }

    protected String getAsTextInternal() {
        return getValue() == null ? "" : String.valueOf(((Seconds) getValue()).getSeconds());
    }
    ...
    private Seconds parse(Number number) {
